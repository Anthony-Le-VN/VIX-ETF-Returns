...
- The repository is organized as a complete and self-contained replication package designed to reproduce all empirical results in the final paper. At the top level, the project includes the finalized paper in PDF format, which presents the research question, methodology, and findings, along with documentation files that describe the structure and usage of the repository.
- All data-related materials are contained in the Data/ directory, which is divided into clearly defined stages of the empirical workflow. The Original_Data/ folder stores raw and lightly processed Bloomberg exports, along with a metadata subfolder containing a detailed codebook describing variable construction. The Analysis_Data/ folder contains the finalized Stata dataset used for all regressions and figures. Outputs generated by the analysis—such as regression tables, summary statistics, and visualizations—are stored separately in the Regression_Output/ and Visualization/ subdirectories to maintain a clean separation between inputs and results.
- The empirical workflow is driven by Stata scripts located in the Command_Files/ directory. These include scripts for data processing and variable construction as well as scripts for the main econometric analysis. Corresponding log files are provided to document execution and ensure transparency. Together, this structure follows best practices for reproducible empirical research by clearly separating raw data, processed data, code, outputs, and documentation.
